...
class BreakStartDialog(ctk.CTkToplevel):
    def __init__(self):
        super().__init__()
        
        # Configurações básicas da janela
        self.title("Início do Intervalo")
        ...
        try:
            ...
            alert_path = os.path.join(icons_dir, 'lunch-time.png')
        ...
        self.message_label = ctk.CTkLabel(
            self,
            text=f"Seu intervalo começa às {break_start.strftime('%H:%M')} e termina às {break_end.strftime('%H:%M')}\n"
                 "Não se esqueça de registrar o retorno!",
            font=("Roboto", 14),
            justify="center"
        )
