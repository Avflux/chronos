...
class AdminWindow(BaseWindow, TimeObserver):
    ...
    def setup_ui(self):
        ...
        # Frame de atividades
        info_frame = ctk.CTkFrame(self.top_frame, height=50)
        ...
        # Inicialmente, mostrar a tela de atividades
        self.show_activities()
    ...
    def load_statistics(self):
        ...
        try:
            ...
            queries = {
                "Total de Usuários": "SELECT COUNT(*) as count FROM usuarios",
                "Usuários Ativos": "SELECT COUNT(*) as count FROM usuarios WHERE status = TRUE",
                "Total de Equipes": "SELECT COUNT(*) as count FROM equipes",
            }
            ...
            messagebox.showerror("Erro", "Erro ao carregar estatísticas")
    ...
    def show_register_dialog(self):
        ...
        UserManagementFrame(self, self.user_data)
    ...
    def update_activity_status(self, activity_info: Optional[Dict]) -> None:
        ...
        try:
            if activity_info:
                ...
                time_status = self.time_manager.check_company_hours()
    ...
    def notify_time_exceeded(self, activity_info: Dict) -> None:
        ...
        try:
            # Notificar através do notification manager
    ...
    def _pause_active_activities(self):
        ...
        try:
            ...
            for activity in active_activities:
                self.activity_controls.handle_activity_action("pausar")
    ...
    def show_activities(self):
        ...
        # Frame de atividades com configuração explícita
        activities_frame = ctk.CTkFrame(
            self.main_area,
            fg_color=self.main_area.cget("fg_color")  # Herda cor do pai
        )
        activities_frame.pack(fill="both", expand=True)
        
        # Força atualização antes de criar controles
        self.update_idletasks()
        
        # Passar referências dos labels
        self.activity_controls = ActivityControls(
            activities_frame,
            self.user_data,
            self.db,
            self.handle_activity_action,
            active_label=self.active_activity_label,    # Passa referência do label
            selected_label=self.selected_activity_label # Passa referência do label
        )
    ...
    def update_idle_status(self, status: str):
        ...
        try:
            if status == 'idle':
                ...
                logger.debug("Usuário está inativo (AdminWindow)")
            else:
                # Lógica para quando o usuário retoma a atividade
                logger.debug("Usuário está ativo (AdminWindow)")
    ...
    def open_user_management(self):
        ...
        # Limpar o frame atual se existir
